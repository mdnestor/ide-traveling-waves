...
\begin{proposition}
Suppose $r\leq r_m$.
Then there exists a unique $c^*$ such that there exists a nonincreasing traveling wave $w \in C(\mathbb{R},[0,1])$ such that
$$ Qw(x)=w(x-c^*) $$
with $w(-\infty)=1$ and $w(\infty)=0$.
\end{proposition}
...
\begin{proposition}
Suppose $r_1 < r \leq r_m^1$ (so that $g^2$ is monotone on $[u^-,u^=]$). Then there exists a {\it spreading speed} $c^*_{(1,u^+)}$ for the double-iterate operator $Q^2$ from $1$ to $u^+$. This means:
\begin{enumerate}
\item for any initial data $u_0 \in C(\mathbb{R},[1,u^+])$ with $u_0(x)=1$ outside a compact set, then
$$ \lim_{t\to\infty} \sup_{|x| \geq ct} Q^{2t} u_0(x) = 1, \quad \forall c > c^* $$
\item for any initial data $u_0 \neq 1$,
$$ \lim_{t\to\infty} \inf_{|x| \leq ct} Q^{2t}u_0(x) = u^+, \quad \forall c \in (0,c^*) $$
\end{enumerate}
\end{proposition}
...
\begin{proposition}
If $r_1 < r \leq r_m^1$, then with $c^*$ given in the previous theorem, there exists a monotone traveling wave solution $u_t(x)=w(x-ct)$ such that $w(-\infty)=u^+$ and $w(\infty)=1$.
\end{proposition}
...
\subsection{Stable $2$-cycle and step initial data}
...
Our basic observation, as depicted in \autoref{fig:fig1}, is that the solution falls in one of two cases:
\begin{enumerate}
\item converges uniformly to a $2$-periodic traveling wave connecting $u^\pm$ to $0$, that is, we have
$$ u_t(x) - w_t(x-ct) \xrightarrow{\text{unif.}} 0$$
for some $w_t$ and $c$ satisfying:
\begin{enumerate}
\item $w_0(-\infty)=u^+$,
\item $w_0(\infty)=0$,
...
To distinguish these cases, we define the following sequence of front-tracking points, depending on $\epsilon>0$:
...
\subsection{Comments on wave trains}
...
where $w_t$ is a periodic traveling wave profile with velocity $c$.
\end{enumerate}
